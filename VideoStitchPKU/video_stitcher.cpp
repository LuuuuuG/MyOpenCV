...
MyVideoStitcher::MyVideoStitcher()
{
	...
	work_megapix_ = 0.6; // 1.0;//-1;//
	seam_megapix_ = 0.1; // 0.2;//-1;//
	is_prepared_ = false;
	conf_thresh_ = 0.01f; // 0.1f; // 0.6f;// 1.f;//
	features_type_ = "surf";// "orb";//"surf";//
	ba_cost_func_ = "ray";
	ba_refine_mask_ = "xxxxx";
	is_do_wave_correct_ = true;
	wave_correct_ = detail::WAVE_CORRECT_HORIZ;
	is_save_graph_ = false;
	warp_type_ = "plane";//"cylindrical";//"plane";//"apap";//"paniniA2B1";//"transverseMercator";//"spherical";//
	expos_comp_type_ = ExposureCompensator::GAIN_BLOCKS;//ExposureCompensator::GAIN;//
	match_conf_ = 0.65;// 0.3f;
	seam_find_type_ = "gc_color";//"voronoi";//
	blend_type_ = Blender::MULTI_BAND;//Blender::FEATHER;//Blender::NO;//
	blend_strength_ = 5;
...
int MyVideoStitcher::stitch( vector<VideoCapture> &captures, string &writer_file_name )
{
	...
	if (is_preview_){
		...
		resizeWindow(window_name, 2048, 768);
	}
...
int MyVideoStitcher::FindSeam(vector<Mat> &images_warped, vector<Mat> &masks_warped)
{
	...
	for (int i = 0; i < num_images; ++i) {
		...
		masks_warped2[i] = masks_warped[i].getUMat(ACCESS_RW);
	}
	...
	seam_finder->find(images_warped_f, corners_, masks_warped2);
