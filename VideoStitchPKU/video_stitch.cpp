...
static string warp_type = "plane"; //"apap";// "cylindrical";// 
...
static int parseCmdArgs(int argc, char* argv[])
{
	...
	*/
	...
	video_names.clear();
#if 1
	video_names.push_back("E:\\videos\\trans\\VideoL2.avi");
	video_names.push_back("E:\\videos\\trans\\VideoR2.avi");
#else
	is_camera = true;
#endif
	cam_num = 2;
	cam_width = 1280; //704;//1920;
	cam_height = 720; //576;//   1080;
...
static int VideoStitch(int argc, char* argv[])
{
	...
	if(is_camera)
	{
		for(int cam_idx = 0; cam_idx < cam_num; cam_idx++)
		{
			...
			String cam_url;
			if (cam_idx == 0) {
				cam_url = "rtsp://admin:123456@172.17.30.101:554/cam/realmonitor?channel=1&subtype=0";
			}
			else {
				cam_url = "rtsp://admin:123456@172.17.30.100:554/cam/realmonitor?channel=1&subtype=0";
			}
			//if(cam_cap.open(cam_idx))
			if (cam_cap.open(cam_url))
			{
