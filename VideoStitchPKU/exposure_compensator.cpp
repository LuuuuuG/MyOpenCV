	...
	for (int img_idx = 0; img_idx < num_images; ++img_idx)
	{
		...
		for (int by = 0; by < bl_per_img.height; ++by)
		{
			for (int bx = 0; bx < bl_per_img.width; ++bx)
			{
				...
				block_images.push_back((images[img_idx](Rect(bl_tl, bl_br))).getUMat(ACCESS_RW));
				block_masks.push_back(make_pair((masks[img_idx].first(Rect(bl_tl, bl_br))).getUMat(ACCESS_RW), masks[img_idx].second));
			}
		}
	}
